...
### Изменение основного словаря

В данный момент основной словарь составлен как объединение двух независимых словарей (см. раздел "Описание выбранного алгоритма" выше). Этот словарь можно менять, внося изменения непосредственно в файлы словаря, изменяя при этом состав, но не формат словаря. Можно переносисть слова из словаря ударных форм в словарь омографов и обратно, учтя разницу в организации этих словарей. Однако, можно изменить основной словарь и массово, например, объединив его с другим словарем. 

Анализ нескольких словарей, которые потенциально могут быть интересны как источники для массового расширения основного словаря, показал, что этот процесс крайне сложно унифицировать: особенности того или иного добавляемого словаря не позволяют предусмотреть заранее все тонкости и предусмотреть быстрый и качественный механимз объединения. Так как в целом потенциальных словарей для добавления не так много и польза от добавления каждого следующего словаря уменьшается (вероятность извлечь новые слова меньше с каждым новым добавляемым словарем), проще подходить к этой задаче в индивидуальном порядке и разрабатывать отдельный код для каждого из выбранных словарей. Трудоемкость добавления словаря в основной словарь (то есть исправление и файла правильных слов, и файла омографов) мы оценили в 6 человеко-часов на примере проведенной работы по объединению двух словарей в текущий основной словарь. Как основной иструмент для этой работы предлагаем рассмотреть python и библиотеку Pandas.
...
![image](https://github.com/KirillStarodubovVR/CutEggOff/assets/89096305/7a076aa0-b70c-4c8a-b922-536bde3b0713)
...
Используется библиотека customtkinter для создания пользовательских интерфейсов. Интерфейс использовать можно только локально и он запускается из файла ```pc_app.py```.
Предусмотрены варианты внесения отдельного предложения, так и загрузки файлов.

![image](https://github.com/KirillStarodubovVR/CutEggOff/assets/89096305/03e66af0-a9d4-429d-a807-b5b9cfd43477)
